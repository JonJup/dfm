## -- Plot classification strength w multiple nulls

# created: 16.12.22
# modified: 16.12.22
# project : Evaluating European Broad River Types for Diatoms, Fish and Macrophytes
# purpose : visualize the results of the classification strength analysis with multiple nulls 
...
### log files ---- 
sink(file = paste0("02_R/999_log_files/classification_strength_plot_w_multiple_null", Sys.Date(), "_", "log.txt"))
...
vlinevar1 <- cs[typology == "null_model1_type" & resolution == "species"]
vlinevar1 %<>% unique(by = c("typology", "resolution", "taxon"))
vlinevar2 <- cs[typology == "null_model2_type" & resolution == "species"]
vlinevar2 %<>% unique(by = c("typology", "resolution", "taxon"))
vlinevar3 <- cs[typology == "null_model3_type" & resolution == "species"]
vlinevar3 %<>% unique(by = c("typology", "resolution", "taxon"))
vlinevar4 <- cs[typology == "null_model4_type" & resolution == "species"]
vlinevar4 %<>% unique(by = c("typology", "resolution", "taxon"))
out_plot <- cs |>  
        ...
        geom_hline(aes(yintercept = classification_strength), data = vlinevar1, lty = "dashed", linewidth = 1, col = "red") + 
        geom_hline(aes(yintercept = classification_strength), data = vlinevar2, lty = "dashed", linewidth = 1) + 
        geom_hline(aes(yintercept = classification_strength), data = vlinevar3, lty = "dashed", linewidth = 1) + 
        geom_hline(aes(yintercept = classification_strength), data = vlinevar4, lty = "dashed", linewidth = 1) + 
        xlab(NULL) + 
        ...
        theme(panel.background = element_rect(fill = "white"),
              ...
              panel.border = element_blank(),
              panel.spacing.y = unit(0,"line")) 
ggsave(filename = paste0("04_fig/002_results/002_classification_strength/cs_all_w_multiple_nulls", Sys.Date(), ".tiff"))
