## -- plot zeta results with multiple null models
...
### log files ---- 
sink(file = paste0("02_R/999_log_files/zeta_plot_multiple_null", Sys.Date(), "_", "log.txt"))
...
plot_fun <- function(res){
        ...
        vlinevar1 <-  data[typology == "null_model1_type" & taxonomic_resolution == res] %>%  unique(by = c("typology", "taxonomic_resolution", "taxon","type"))
        vlinevar2 <-  data[typology == "null_model2_type" & taxonomic_resolution == res] %>%  unique(by = c("typology", "taxonomic_resolution", "taxon","type"))
        vlinevar3 <-  data[typology == "null_model3_type" & taxonomic_resolution == res] %>%  unique(by = c("typology", "taxonomic_resolution", "taxon","type"))
        vlinevar4 <-  data[typology == "null_model4_type" & taxonomic_resolution == res] %>%  unique(by = c("typology", "taxonomic_resolution", "taxon","type"))
        
        vlinevar1[, auc := median(auc), by = c("typology", "taxon", "taxonomic_resolution")]
        vlinevar2[, auc := median(auc), by = c("typology", "taxon", "taxonomic_resolution")]
        vlinevar3[, auc := median(auc), by = c("typology", "taxon", "taxonomic_resolution")]
        vlinevar4[, auc := median(auc), by = c("typology", "taxon", "taxonomic_resolution")]
        
        vlinevar1 %<>% unique(by =  c("typology", "taxon", "taxonomic_resolution"))
        vlinevar2 %<>% unique(by =  c("typology", "taxon", "taxonomic_resolution"))
        vlinevar3 %<>% unique(by =  c("typology", "taxon", "taxonomic_resolution"))
        vlinevar4 %<>% unique(by =  c("typology", "taxon", "taxonomic_resolution"))
        
        
        
        out_plot <- 
                data |>  
                filter(!str_detect(typology,"null_model"), taxonomic_resolution == res) |> 
                ...
                geom_hline(aes(yintercept = auc), data = vlinevar1, lty = "dashed", linewidth = 1, col = "red") + 
                geom_hline(aes(yintercept = auc), data = vlinevar2, lty = "dashed", linewidth = 1) + 
                geom_hline(aes(yintercept = auc), data = vlinevar3, lty = "dashed", linewidth = 1) + 
                geom_hline(aes(yintercept = auc), data = vlinevar4, lty = "dashed", linewidth = 1) + 
                xlab(NULL) + 
...
data <- 
        ...
        mutate(typology = factor(typology, levels = c("brt", "bgr", "enz", "few", "ife", "null_model1_type", "null_model2_type", "null_model3_type", "null_model4_type")))
...
# save to file ----------------------------------------------------------------------
ggsave(plot = plot_sp, 
       filename = "04_fig/002_results/005_zeta/221212_zeta_species_multiple_nulls.png")
