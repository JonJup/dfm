...
library(pacman)
p_load(ggplot2, dplyr, data.table, magrittr, wesanderson, stringr)

### log files ---- 
sink(file = paste0("02_R/999_log_files/indval_plot_multiple_null", Sys.Date(), "_", "log.txt"))
...
plot_fun <- function(resolution){
        ...
        vlinevar1 <- data[typology == "null_model1" & taxonomic.resolution == resolution]
        vlinevar1 %<>% unique(by = c("typology", "taxonomic.resolution", "taxon"))
        
        vlinevar2 <- data[typology == "null_model2" & taxonomic.resolution == resolution]
        vlinevar2 %<>% unique(by = c("typology", "taxonomic.resolution", "taxon"))
        
        vlinevar3 <- data[typology == "null_model3" & taxonomic.resolution == resolution]
        vlinevar3 %<>% unique(by = c("typology", "taxonomic.resolution", "taxon"))
        
        vlinevar4 <- data[typology == "null_model4" & taxonomic.resolution == resolution]
        vlinevar4 %<>% unique(by = c("typology", "taxonomic.resolution", "taxon"))
        
        
        out_plot <- 
                data |>  
                filter(!str_detect(typology, "null_model"), taxonomic.resolution == resolution) |> 
                unique(by = c("typology", "taxonomic.resolution", "taxon")) |> 
                ggplot(aes(y=n.indicator.adj, x=typology)) + 
                geom_col(aes(fill = typology), alpha = .8) +
                geom_hline(aes(yintercept = n.indicator.adj), data = vlinevar1, lty = "dashed", linewidth = 1, col = "red") + 
                geom_hline(aes(yintercept = n.indicator.adj), data = vlinevar2, lty = "dashed", linewidth = 1) + 
                geom_hline(aes(yintercept = n.indicator.adj), data = vlinevar3, lty = "dashed", linewidth = 1) + 
                geom_hline(aes(yintercept = n.indicator.adj), data = vlinevar4, lty = "dashed", linewidth = 1) + 
                xlab(NULL) + 
...
plot_fun2 <- function(resolution){
        ...
        out_plot <- 
                ...
                geom_col(aes(fill = typology), alpha = .8) +
                geom_hline(aes(yintercept = mean_p), data = vlinevar1, lty = "dashed", linewidth = 1, col = "red") + 
                geom_hline(aes(yintercept = mean_p), data = vlinevar2, lty = "dashed", linewidth = 1) + 
                geom_hline(aes(yintercept = mean_p), data = vlinevar3, lty = "dashed", linewidth = 1) + 
                geom_hline(aes(yintercept = mean_p), data = vlinevar4, lty = "dashed", linewidth = 1) + 
                xlab(NULL) + 
                ylab("Mean p-value") + 
                facet_grid(.~taxon) + 
                scale_color_manual(values = pal, guide = "none") +
...
data[, typology := factor(typology, levels = c("brt", "bgr", "enz", "feow", "ife", "null_model1", "null_model2", "null_model3", "null_model4"))]
...
for (i in c("species", "genus", "family")){
        plot_fun(i); ggsave(paste0("04_fig/002_results/003_identicator/indval_n_",i,"_with_mutliple_nulls_", Sys.Date(),".tiff"))  
}
for (i in c("species", "genus", "family")){
        plot_fun2(i); ggsave(paste0("04_fig/002_results/003_identicator/indval_p_",i,"_with_multiple_nulls_", Sys.Date(),".tiff"))  
}
