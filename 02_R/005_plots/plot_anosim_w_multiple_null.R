## -- plot anosim results w multiple null

# created: 16.12.22
# modified: 16.12.22
# project : Evaluating European Broad River Types for Diatoms, Fish and Macrophytes
# purpose : visualize the results of the ANOSIM analysis with mutliple null models
...
### log files ----      
sink(file = paste0("02_R/005_plots/log_files/anosim_w_multiple_null", Sys.Date(), "_", "log.txt"))
...
plot_out <- function(res, tax = "all"){
        ...
        vlinevar1 <- data[typology == "null_model1_type"& taxonomic.resolution == res & taxon %in% tax]
        vlinevar2 <- data[typology == "null_model2_type"& taxonomic.resolution == res & taxon %in% tax]
        vlinevar3 <- data[typology == "null_model3_type"& taxonomic.resolution == res & taxon %in% tax]
        vlinevar4 <- data[typology == "null_model4_type"& taxonomic.resolution == res & taxon %in% tax]
        ...
        out_plot <- 
                ...
                geom_hline(aes(yintercept = statistic), data = vlinevar1, lty = "dashed", linewidth = 1, col = "red") + 
                geom_hline(aes(yintercept = statistic), data = vlinevar2, lty = "dashed", linewidth = 1) + 
                geom_hline(aes(yintercept = statistic), data = vlinevar3, lty = "dashed", linewidth = 1) + 
                geom_hline(aes(yintercept = statistic), data = vlinevar4, lty = "dashed", linewidth = 1) + 
                xlab(NULL) + 
                ylab("ANOSIM R") + 
                facet_grid(.~taxon) + 
...
plot_out(res = "species"); ggsave(filename = paste0("04_fig/002_results/001_anosim/anosim_all_w_multiple_null_species", Sys.Date(), ".tiff"))
plot_out(res = "genus")  ; ggsave(filename = paste0("04_fig/002_results/001_anosim/anosim_all_w_multiple_null_genus", Sys.Date(), ".tiff"))
plot_out(res = "family") ; ggsave(filename = paste0("04_fig/002_results/001_anosim/anosim_all_w_multiple_null_family", Sys.Date(), ".tiff"))
