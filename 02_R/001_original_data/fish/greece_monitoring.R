...
# load data ----------------------------------------------------------------------
bio <- read_excel("data/fish/original_data/greece_monitoring/raw/fish Greece.xlsx") 
...
## drop columns 
bio2 %<>% d$select(- c(2,5,6,7))
bio2 %<>% d$select(- `Grand Total`)
setDT(bio2)
bio2[, date := lubridate::ymd(Date)]
bio2[, date_id := .GRP, by = date]
bio2[, site_id := .GRP, by = Site]
...
bio2 %<>% d$select(!Date)
bio2 %<>% d$rename(x.coord = Longtitude)
bio2 %<>% d$rename(y.coord = latitude)
bio2 %<>% d$rename(original_site_name = Site)
bio2 %<>% d$filter(ID != "Grand Total")
bio3 <- pivot_longer(bio2, cols = !c("original_site_name", "site_id", "date_id", "date", "season", "x.coord", "y.coord", "year", "month", "ID"), names_to = "taxon", values_to = "abundance")
bio3 %<>% d$filter(!is.na(abundance))

bio3 |> d$filter(is.na(x.coord))
...
setDT(bio3)
bio3[, EPSG := 4326]
bio3[, data.set := "greece_monitoring_fish"]
data <- bio3 
rm(bio2, bio3, sites)
...
#taxontable_add <- jjmisc::update_taxonomy2(TU)
#taxontable <- taxontable_add
...
## add gr_sample_id
data2[,gr_sample_id := paste0("site_", site_id, "_date_", date_id,"_greece_monitroting_fish")]
...
data3 <- data2[, list(
        ...
        y.coord,
        EPSG,
        data.set = "greece_monitoring_fish"
)]
...
updated_type_old <- readRDS("data/fish/original_data/greece_monitoring/2022-03-04_updated_type.rds")

## how many sites in data8: 15
uniqueN(data8$site_id)
## sites covered in updated_type_old: 14 
...
rt <- 
        data8 |> 
        d$filter(!site_id %in% updated_type_old$site_id) |> 
        unique(by = "site_id") |> 
...
updated_type_combi <- d$bind_rows(updated_type, updated_type_old)

#- save the remove list. 
saveRDS(updated_type_combi, paste0("data/fish/original_data/greece_monitoring/", Sys.Date(), "_updated_type_combi.rds"))
...
# - save to file 
saveRDS(data10, paste0("data/fish/original_data/greece_monitoring//",Sys.Date(),"_final_aggregated.rds"))
