...
# Purpose: Clean fish data provided by Apostolos Apostolou for Bulgaria
# Notes: 
# -------------------------------
...
bio <- read_excel("data/fish/original_data/bulgaria_monitoring/raw/FISHspecies_sum_ICSites.xlsx")
...
bio[, c("RivType", "Species", "ind/ha", "SiteName_EN") := NULL]

data2 <- data.table (
        original_site_name = bio$SiteCode,
        date = lubridate::ymd(bio$DateSampling),
        taxon = bio$EFI_Name, 
        abundance = bio$Count, 
        x.coord = bio$E, 
        y.coord = bio$N, 
        waterbody = bio$River_EN,
        bio.set = "bulgaria_monitoring_fish",
        EPSG = 4326
)
## add season and year 
...
data2 <- data2[!taxon %in% c("No fish", "No name")]
...
## add gr_sample_id
data3[,gr_sample_id := paste0("site_", site_id, "_date_", date_id, "_bulgaria_monitoring_fish")]
...
data4 <- data3[, list(
        ...
        y.coord,
        EPSG,
        data.set = "bulgaria_monitoring_fish",
        waterbody
)]
...
updated_type_old <- readRDS("data/fish/original_data/bulgaria_monitoring/2022-03-07_updated_type.rds")

## how many sites in data8: 27
uniqueN(data8$site_id)
## sites covered in updated_type_old: 19  
...
saveRDS(updated_type_combi, paste0("data/fish/original_data/bulgaria_monitoring/", Sys.Date(), "_updated_type_combi.rds"))
...
# - save to file 
saveRDS(data10, paste0("data/fish/original_data/bulgaria_monitoring/",Sys.Date(),"_final_aggregated.rds"))
