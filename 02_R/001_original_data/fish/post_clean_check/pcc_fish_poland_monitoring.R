...
# Purpose: Post Clean Check for Fish data from Greece Moitoring
# Notes: 
# -------------------------------
...
# load data -------------------------------------------
data <- readRDS("data/fish/combined_data/01_2022-03-04_combined_data_aggregated.rds")
data <- rbindlist(data)
unique(data$data.set) |> sort()
data <- data[data.set == "poland_monitoring_fish"]

data_name <- "fish_poland_monitoring"
