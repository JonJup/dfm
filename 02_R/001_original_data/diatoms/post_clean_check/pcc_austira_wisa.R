...
data <- readRDS("data/diatoms/original_data/austria/2022-01-19_final_aggregated.rds")
...
lcbd |> 
        group_by(level) %>%
        mutate(outlier = ifelse(is_outlier(lcbd), id, as.numeric(NA))) %>%
...
for (i in 1:3){
       ...
       p[[i]] <- 
               ...
               ggtitle(i.title) + 
               geom_text(aes(label = outlier, col = brt), na.rm = TRUE)
...
for (i in 1:3){
       ...
       i.title <- switch(i, "1" = "species", "2" = "genus", "3" = "family")        
       i.x <- nmds.n[[i]]  
       p[[i]] <- ggplot(i.x, aes(x, y)) + geom_point(aes(col = brt)) + geom_polygon(data = hull.n[[i]], alpha = 0.5, aes ( fill = brt)) + ggtitle(i.title)
       rm(list = ls()[grepl(pattern = "^i\\.", x = ls())])
...
for (i in 1:3){
       ...
       i.title <- switch(i, "1" = "species", "2" = "genus", "3" = "family")        
       i.x <- nmds.j[[i]]  
       p[[i]] <- ggplot(i.x, aes(x, y)) + geom_point(aes(col = brt)) + geom_polygon(data = hull.j[[i]], alpha = 0.5, aes ( fill = brt)) + ggtitle(i.title)
       rm(list = ls()[grepl(pattern = "^i\\.", x = ls())])
