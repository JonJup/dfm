# —————————————————————————————————————————————————————————— #
# ——— Clean Diatom data from Germany, Schleswig-Holstein ——— # 
# —————————————————————————————————————————————————————————— #
...
# raw data from Germany Schelswig-Holstein.  
...
bio_wd <- "data/diatoms/original_data/germany_schleswig_holstein/raw/SH_Phytoplankton_2016-2020.xlsx"
...
bio2 <- 
        bio |> 
        filter(Algenklasse == "Bacillariophyceae") |> 
        select(original_site_name = 'Messstelle Nr.', 
               date = Datum,
               taxon = 'wissenschaftl. Artname',
               abundance = Abundanz,
               x.coord = 'UTM-32 East', 
               y.coord = 'UTM-32 North'
               ) |> 
        mutate( 
                data.set = "germany_schleswig_holstein_diatoms",
                EPSG = 4647,
                date = dmy(date)
                ) |> 
        setDT()
...
data <- data[!taxon %in% c("Centrales","Pennales")]
...
## add gr_sample_id
data2[,gr_sample_id := paste0("site_", site_id, "_date_", date_id,"_germany_schleswig_holstein_diatoms")]
...
saveRDS(remove_list, paste0("data/diatoms/original_data/germany_schleswig_holstein/", Sys.Date(), "_remove_list.rds"))
...
data10
saveRDS(data10, paste0("data/diatoms/original_data/germany_schleswig_holstein/",Sys.Date(),"_final_aggregated.rds"))
