# ———————————————————————————————————————————————————————————————————————— #
# ——— Clean Diatom data from Germany - Mecklenburg-West Pomerania  ——————— # 
# ———————————————————————————————————————————————————————————————————————— #

# ———————————————————————————————————
# date first written: 22-12-21
# date last modified: 22-12-21
# Project: Evaluating European Broad River Types for Diatoms, Fish and Macrophytes
# Purpose: In this script I create a harmonized spatial data set of diatom from the 
# raw data from Germany - Mecklenburg-West Pomerania.   
...
bio_wd <- "data/diatoms/original_data/germany_mecklenburg/raw/Diatomeen.xlsx"

bio <- read_excel(bio_wd, sheet = 2) 
sit <- read_excel(bio_wd, sheet = 1)
...
setDT(bio)
bio2 <- bio[, .(original_site_name = Messstelle,
                date = as.Date(NA), 
                year = Jahr,
                taxon = Taxonname,
                abundance = Messwert)]
...
## add gr_sample_id
data2[,gr_sample_id := paste0("site_", site_id, "_date_", date_id,"_germany_mecklenburg_diatoms")]
...
saveRDS(remove_list, paste0("data/diatoms/original_data/germany_mecklenburg/", Sys.Date(), "_remove_list.rds"))
...
data10
saveRDS(data10, paste0("data/diatoms/original_data/germany_mecklenburg/",Sys.Date(),"_final_aggregated.rds"))
