...
bio <- read_excel("data/macrophytes/original_data/slovakia/raw/macrophyte_data_Slovakia.xlsx")
taxontable <- readRDS("data/macrophytes/2022-06-14_taxontable_macrophytes.rds")
typologies <- readRDS("data/all_typologies.rds")

# prepare data ----------------------------------------------------------------------

bio %<>% dp$filter(!is.na(`taxon name`))
bio %<>% dp$filter(`taxonomic group` != "Algae")
...
# taxontable[original_name %in% c("Dichelymoides sp.", "Pteropsida", "Zbiorowisko z Leptodictyum riparium", "Zbiorowisko z Platyhypnidium riparioides"), clean := TRUE]
# taxontable[clean == FALSE]
# taxontable[original_name == "Ranunculus peltatus"]
# taxontable[original_name == taxontable[clean == FALSE, original_name][1], 
#            c("species", "genus", "family", "order", "subclass", "class","phylum", "kingdom", "clean", "taxon_state") := 
#                    .("Ranunculus peltatus", "Ranunculus", "Ranunculaceae", "Ranunculales", NA, "Magnoliopsida", "Tracheophyta", "Plantae",  TRUE, "hydrophytes")]
# taxontable[original_name == taxontable[clean == FALSE, original_name][1], 
#            c("species",                    "genus",          "family",           "order",  "subclass", "class",     "phylum",    "kingdom", "clean", "taxon_state") := 
#            .("Platyhypnidium riparioides", "Platyhypnidium", "Brachytheciaceae", "Hypnales", NA,       "Bryopsida", "Bryophyta", "Plantae", TRUE,    "moss")
# ]
# taxontable[original_name == taxontable[clean == FALSE, original_name][1], 
#            c("species",            "genus",    "family",          "order",             "subclass", "class",         "phylum",       "kingdom", "clean", "taxon_state") := 
#            .("Myosoton aquaticum", "Myosoton", "Caryophyllaceae", "Caryophyllales",     NA,        "Magnoliopsida", "Tracheophyta", "Plantae",  TRUE,        "drop")
# ]
# taxontable[original_name == taxontable[clean == FALSE, original_name][1], 
#            c("species",            "genus",    "family",          "order",             "subclass", "class",         "phylum",       "kingdom", "clean", "taxon_state") := 
#            .("Thamnobryum alopecurum", "Thamnobryum", "Neckeraceae", "Leucodontales",     NA, "Bryopsida", "Bryophyta", "Plantae",  TRUE,        "moss")
# ]
# 
...
uto <- readRDS("data/macrophytes/original_data/slovakia/2022-04-22_updated_type.rds")
...
saveRDS(updated_type, paste0("data/macrophytes/original_data/slovakia/", Sys.Date(), "_updated_type.rds"))
...
saveRDS(data10, paste0("data/macrophytes/original_data/slovakia/",Sys.Date(),"_final_aggregated.rds"))
