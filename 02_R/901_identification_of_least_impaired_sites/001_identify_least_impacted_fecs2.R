...
for (i in nrow(brt2):1) {
        ...
        if (nrow(i.diff) == 0) {
                ...
                i.diff2 <- st_length(i.diff) / st_length(brt[i, ])
        }
        ...
        if (i%%500 == 0) {
                ...
                saveRDS(brt2, paste0("data/quick_save_least_disturbed_delete_me/quicksave_from_top",i,".rds"))
        }
        
        rm(i)
        rm(list = ls()[grepl(pat tern = "^i\\.", x = ls())])
}
